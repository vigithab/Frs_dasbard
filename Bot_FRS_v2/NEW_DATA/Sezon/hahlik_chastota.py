...
BOT.BOT().bot_mes_html(mes="Скрипт Частота запущен",silka=0)
try:
    ...
    for f_ch in os.listdir(path_cheks):
        ...
        if m in spisok_m_number:
            ...
            memory.MEMORY().mem_total(f"В цикле: {f_ch}")
            del df_ch
            gc.collect()
    table=pd.DataFrame()
    for nsi in spr_list:
    ...
    if  len(spisok_m_number) == 1:
        ...
        table.to_csv(r'P:\Фирменная розница\ФРС\Данные из 1 С\Шашлык\2023-Count.txt', sep='\t', index=False,
                     decimal=",")
    log.LOG().log_new_data(name_txt="шашлыччный частота")
except Exception as e:
    exc_type, exc_obj, exc_tb = sys.exc_info()
    mes = f"Ошибка при скачивании : {exc_type.__name__} на строке {exc_tb.tb_lineno}: {e}\n"
    log.LOG().log_new_data(name_txt="шашлыччный частота", e=mes)
    BOT.BOT().bot_mes_html(mes="Ошибка при шашлыччный частота", silka=0)