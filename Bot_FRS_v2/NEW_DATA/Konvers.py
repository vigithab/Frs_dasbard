...
class konvers():
    ...
    def selenium_day_chek(self):
        #for root, dirs, files in os.walk(ini.PUT + "Selenium\\Оригинальные файлы\\"):
        """for file in files:
            if "2023" in file:
                file_name, file_extension = os.path.splitext(file)
                try:
                    file_date = datetime.datetime.strptime(file_name, "%d.%m.%Y")
                    if file_date.month == ini.month_todey:"""
        all_files = []
        for root, dirs, files in os.walk(ini.PUT + "♀Планы\\Исходник\\"):
            for file in files:
                all_files.append(os.path.join(root, file))
            for i in all_files:
                dat = os.path.basename(i)[:-5]
                dat = datetime.datetime.strptime(dat, "%d.%m.%Y")
                print(dat)
                #file_path = os.path.join(root, file)
                print("Фаил: ", os.path.basename(i)[:-5], " / Начат: ",
                      str(datetime.datetime.now())[:-10])

                name_datafreme = pd.read_excel(i)
