...
class Degustacia:
    def sotka(self):
        if ini.time_seychas <ini.time_bot_vrem:
            ...
            df = pd.DataFrame({"Магазин": unique_values})
            df["магазин_c_дегустацией"] = df["Магазин"]
            ty_open_magaz = ty_open_magaz.rename(columns={"магазин": "Магазин"})
            ty_open_magaz["магазин2"] = ty_open_magaz["Магазин"]
            df = pd.merge(ty_open_magaz,df, on=["Магазин"], how="left").reset_index(drop=True)
            df = df.loc[df["магазин_c_дегустацией"].isnull()]
