...
PUT = setting.PUT
sys.path.append(setting.PUT_python)
#import psutil
import shutil
import xlsxwriter
...
from Bot_FRS.new_data.set_cd import NEW_DATA as sd
from Bot_FRS.new_data.setreteyl import  SET as set
from Bot_FRS.new_data import sort_file as sort
from Bot_FRS.inf import memory as memory
from Bot_FRS.bot_telegram import Bot as bot
from Bot_FRS.rassilka import voropaev_degustaciya as voropaev
from Bot_FRS.inf import NASTROYKA as setting
from Bot_FRS.new_data import nacpitelniy_itog as naK
    ...
    def magazin_info(self):
        ...
        while True:
            try:
                ...
                spqr = pd.read_excel(PUT + "Справочники\\Магазины\\Справочник ТТ.xlsx")
                spqr = spqr[['ID', '!МАГАЗИН!']]
        return spqr
...
class NEW_data:
    def Obrabotka(self):
        bot.BOT().bot_mes_html(mes="Обновление данных....",silka=0)
        print(PUT)
        sd().setevoy()
        sd().setevoy_spisania()
        sd().setevoy_degustacia()
        set().Set_obrabotka()
        ...
        #naK.Grup().grups()
        bot.BOT().bot_mes_html(mes="Закончено....", silka=0)
        return
